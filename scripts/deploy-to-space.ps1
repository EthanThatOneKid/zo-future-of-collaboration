...
# Requires ZO_API_KEY (Settings ΓåÆ Advanced ΓåÆ Access Tokens).
#
# Usage:
...
$key = $env:ZO_API_KEY
if (-not $key) {
    Write-Error "ZO_API_KEY is not set. Create a token in Zo Settings ΓåÆ Advanced, then:`n  `$env:ZO_API_KEY = 'zo_sk_...'`n  .\scripts\deploy-to-space.ps1"
}
...
5. get_space_route path=/future-of-collaboration ΓÇö confirm line count >= 1200 and substrings GlobeStage, findCuboidSubdivisions, lg:grid-cols-[300px_1fr].
6. get_space_errors() ΓÇö report any errors.
