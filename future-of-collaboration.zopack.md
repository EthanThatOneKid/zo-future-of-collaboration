...
description: "100-tile collaboration wall powered by the live Zo examples index (green accent)."
...
function GlobeStage({
  ...
  useEffect(() => {
    for (const entry of tileEntriesRef.current) {
      ...
      entry.borderMaterial.color.setHex(isSelected ? 0x00e676 : 0xffffff);
  ...
  useEffect(() => {
    ...
    async function start() {
      ...
      addAtmosphereGlow(1.06, 0x2ce67f, 2.8, 0.38);
      outerGlowMaterial = addAtmosphereGlow(1.14, 0x00e676, 1.5, 0.16);
      ...
      >
        <aside className="border-b border-white/15 bg-[#222] lg:sticky lg:top-0 lg:h-screen lg:border-b-0 lg:border-r">
          <div className="flex items-center justify-between border-b border-white/15 px-4 py-5 lg:block">
            ...
            <div className="font-mono text-xs uppercase tracking-[0.2em] text-white/45 lg:mt-5">prototype · green</div>
          </div>
